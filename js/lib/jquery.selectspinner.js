...
(function ($) {
	...
	var ARROWUP = 40,
		...
		default_options  = {
			...
			overflowWrapperSelector: '.overflow-wrapper',
			dropDownMaxHeight: {
				desktop: 200,
				mobile: 160
			},
	...
	window.isMobile = window.isMobile || (window.matchMedia && window.matchMedia('only screen and (max-width: 768px)').matches);
		...
		var opened = false,
			...
			me = {
				...
				getActive: function () {
					...
					items = me.dropdown.find('div.xdsoft_item');
					items.removeClass('active');
					for ( i = 0, length = items.length; i < length; i += 1 ) {
			            item = items.eq(i); 
			            if ( item.data('value') == me.val() ) {
			                return item.addClass('active');
			            }
			        }
				},
				next: function (nextfollow) {
					var next = nextfollow ? 'next' : 'prev', active, items, i, length, item;
					...
					if (active.length) {
						me.dropdown.find('div.xdsoft_item.active').removeClass('active');
				...
				open: function () {
					var active, visibleNumber, top, wrapperOffset, dropdownOffset;
					...
					visibleNumber = me.dropdown.height() / active.height();
					if ( active.index() > me.list.length - visibleNumber ) {
						top = active.height() * (me.list.length - visibleNumber - active.index());
		            }
		            else {
		            	top = 0;
		            }

		            if ( me.overflowWrapper.length ) {
		            	wrapperOffset = me.overflowWrapper.offset();
		            	dropdownOffset = me.selector.offset();
		            	if ( wrapperOffset.top < dropdownOffset.top - top ) {
		            		top = 0;
		            	}

		            	if ( wrapperOffset.top + me.overflowWrapper.height() < dropdownOffset.top + me.dropdown.height() - top ) {
		            		top = -me.dropdown.height() + me.selector.height();
		            	}
		            }

					if ( isMobile && me.options.direction.mobile ) {
						if ( me.options.direction.mobile === 'top' ) {
							top = -me.dropdown.height() + me.selector.height();
						}
						else if ( me.options.direction.mobile === 'bottom' ) {
							top = 0;
						}
					}
		            me.dropdown.css('top',  top);
				...
				init:  function (options) {
					...
					me.overflowWrapper = $(me.options.overflowWrapperSelector);
					me.updateList();
					me.update();
					me.dropdown
							...
							if (opened) {
								...
								// if (top < 0 || top + actHght > me.dropdown.height()) {
									me.dropdown.trigger('scroll_element.xdsoft_scroller', [
										(active.index(0) * actHght) / (active.parent().height() - me.dropdown.height())
									]);
								// }
							}
						})
					...
					if (me.options.dropDownMaxHeight) {
						if ( !isMobile ) {
							me.dropdown.css({
								maxHeight: me.options.dropDownMaxHeight.desktop + 'px'
							});
						}
						else {
							me.dropdown.css({
								maxHeight: me.options.dropDownMaxHeight.mobile + 'px'
							});
						}
					}
